...
func handleNewPeer(peer *Peer, roomId string) {
	...
	peer.PC.OnICECandidate(func(i *webrtc.ICECandidate) {
		...
		candidateResponse := &ICECandidateResponse{
			Candidate: i.ToJSON(),
			RoomID:    roomId,
			UserID:    peer.ID,
		}

		candidateString, err := json.Marshal(*candidateResponse)
		if err != nil {
		...
		msg := &sarama.ProducerMessage{
			Topic: "webrtc-server-candidates",
			Value: sarama.ByteEncoder(candidateString),
		}
